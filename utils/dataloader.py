...
class DataSetTrain(torch.utils.data.Dataset):
    def __init__(self, atr_folder='', seg_folder = ''):
    ...
    def __getitem__(self, idx):
        ...
        atr = np.loadtxt(self.atr_folder + atr_name).astype(np.float32)
                
        atr = torch.from_numpy(atr).unsqueeze(0)
            
        # create a range of atrophy values
        patr = torch.ones_like(atr)
        ran = -1.5*torch.rand(1) + 2
        patr[atr > 1] *= ran
        patr[atr < 1] /= ran
            
        # get segmentation 
        seg = np.loadtxt(self.seg_folder + 'rectangle_' + atr_name.split('_')[1].split('.')[0] + '.txt').astype(np.float32)
        seg = torch.from_numpy(seg).unsqueeze(0)

        return seg, atr, atr_name
